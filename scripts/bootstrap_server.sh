...
mkdir -p "${APP_ROOT}/backend" "${APP_ROOT}/frontend" "${APP_ROOT}/infra" "${APP_ROOT}/docs" "${APP_ROOT}/scripts" "${APP_ROOT}/backups/postgres" /var/log/quantitative_trading
...
cat > /etc/logrotate.d/quantitative_trading <<'EOF'
/var/log/quantitative_trading/*.log {
    daily
    rotate 14
    compress
    delaycompress
